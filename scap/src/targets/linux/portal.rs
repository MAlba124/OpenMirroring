...
impl<T: blocking::BlockingSender, C: ::std::ops::Deref<Target = T>> OrgFreedesktopPortalRequest
    for blocking::Proxy<'_, C>
{
    fn close(&self) -> Result<(), dbus::Error> {
...
macro_rules! match_response {
    ( $code:expr ) => {
        match $code {
            0 => {}
            1 => bail!("User cancelled the interaction"),
            2 => bail!("The user interaction was ended in some other way"),
            _ => unreachable!(),
        }
    };
}
...
impl<'a> ScreenCastPortal<'a> {
    ...
    fn create_session(&self) -> Result<dbus::Path> {
        ...
        if let Some(res) = response.lock().unwrap().take() {
                ...
                .map(|h| h.0.as_str().map(String::from))
            {
                Some(h) => {
                    let p = dbus::Path::from(match h {
                        Some(p) => p,
                        None => bail!("Invalid session_handle received"),
                    });

                    return Ok(p);
                }
                None => bail!("Did not get session handle"),
            }
        }

        bail!("Did not get response")
    }

    fn select_sources(&self, session_handle: dbus::Path) -> Result<()> {
    ...
    fn start(&self, session_handle: dbus::Path) -> Result<Stream> {
        ...
        if let Some(res) = response.lock().unwrap().take() {
            ...
            match res.results.get("streams") {
                Some(s) => match Stream::from_dbus(s) {
                    Some(s) => return Ok(s),
                    None => bail!("Failed to extract stream properties"),
                },
                None => bail!("Did not get any streams"),
            }
        }
    ...
    pub fn show_cursor(mut self, mode: bool) -> Result<Self> {
