...
fn main() {
    ...
    Command::new("make")
        ...
        .env("GSTREAMER_JAVA_SRC_DIR", "../android-sender/app/src/main/java")
    ...
    println!("cargo:rustc-link-search=native={}/x86_64", gst_android_build_path);
    println!("cargo:rustc-link-search=native={}", gst_libs);
    println!("cargo:rustc-link-search=native={}/app/libs/x86_64", proj_root.display());
    ...
    let linux_x86_64_lib_dir =
        format!("toolchains/llvm/prebuilt/{}-x86_64/lib/clang/{clang_version}/lib/linux/", env::consts::OS);
